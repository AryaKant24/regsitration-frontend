 ...
 *   division           │ string │ Class division (A/B/C)
...
export function buildRegistrationPayload(
  ...
  // Append student metadata fields
  ...
  formData.append('division', metadata.division.trim());
  formData.append('rollNumber', metadata.rollNumber.trim());

  // Append frame count as a convenience field for the backend
  // (the backend can also count the frame_* fields, but this is explicit)
  formData.append('frameCount', String(frames.length));

  // Append each frame as a File object
  // File names are zero-padded (frame_000, frame_001, …) for consistent ordering
  frames.forEach((frame) => {
    const extension = getExtensionFromMime(frame.mimeType);
    const paddedIndex = String(frame.index).padStart(3, '0');
    const fieldName = `frame_${paddedIndex}`;
